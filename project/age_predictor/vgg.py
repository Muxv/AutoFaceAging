...
class VGG(nn.Module):
    def __init__(self, pool='max'):
        ...
        # vgg modules
        self.conv1_1 = nn.Conv2d(3, 64, kernel_size=(3, 3), padding=(1, 1))
        self.conv1_2 = nn.Conv2d(64, 64, kernel_size=(3, 3), padding=(1, 1))
        self.conv2_1 = nn.Conv2d(64, 128, kernel_size=(3, 3), padding=(1, 1))
        self.conv2_2 = nn.Conv2d(128, 128, kernel_size=(3, 3), padding=(1, 1))
        ...
        self.conv5_1 = nn.Conv2d(512, 512, kernel_size=(3, 3), padding=(1, 1))
        self.conv5_2 = nn.Conv2d(512, 512, kernel_size=(3, 3), padding=(1, 1))
        self.conv5_3 = nn.Conv2d(512, 512, kernel_size=(3, 3), padding=(1, 1))
...
def pretrained_vgg(path, device):
    vgg = VGG()

    vgg_state_dict = torch.load(path, map_location=torch.device(device))
    vgg_state_dict = {k.replace('-', '_'): v for k, v in vgg_state_dict.items()}

    # v = torch.load(path, map_location=torch.device(device))
    # states = vgg.state_dict()
    # for layer, weight in v.items():
    #     if layer in states:
    #         states[layer] = v[layer]
